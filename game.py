...
spritekodland = pygame.transform.scale(spritekodland, (90, 25))
...
while True:
    ...
    if pygame.key.get_pressed()[K_a] and x>0 : x = x - movespeed
    if pygame.key.get_pressed()[K_d] and x<=(largura-30): x = x + movespeed
    if pygame.key.get_pressed()[K_w] and y>0: y = y - movespeed
    if pygame.key.get_pressed()[K_s] and y<=(altura-30): y = y + movespeed
    ...
    if pontos >= 2: tela.blit(spritekodland, (20,20))
